globals [freq-list val-list roll-count H]
to setup
  ca
  clear-all-plots
...
to roll-die
  ask turtles [die]
  let roll (random 6) + 1
  if roll = 1 [ draw-die [ [0 0] ] ]
  if roll = 2 [ draw-die [ [-3 3] [3 -3] ] ]
  if roll = 3 [ draw-die [ [-3 3] [0 0] [3 -3] ] ]
...
to calculate-H
  ...
  foreach freq-list [
    let p_i (? / roll-count)
    if p_i > 0 [set sum-plogp (sum-plogp + (p_i * (log p_i 2)))]
  ]
  set H (sum-plogp * -1)
end
@#$#@#$#@
